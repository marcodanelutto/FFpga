...
//             FileZip * file = new FileZip();

//             // appends the suffix to the filename
//             char suffix[16] = "txt";
//             char _filename[256];
//             snprintf(_filename, 256, "txt/%s.%s", filename, suffix);
...
//             }
//             const size_t size_in_bytes = file_size;

//             // const size_t size_in_bytes = MAX_BUF_SIZE;
//             // const size_t size_in_bytes = (file_size > MAX_BUF_SIZE ? MAX_BUF_SIZE : file_size);
            
            
//             // allocates task
//             int ret = 0;
//             char * txt; ret |= posix_memalign((void **)&txt, 4096, size_in_bytes);
//             char * zip; ret |= posix_memalign((void **)&zip, 4096, size_in_bytes);
//             int * zipsize; ret |= posix_memalign((void **)&zipsize, 4096, sizeof(int));
//             int * txtsize; ret |= posix_memalign((void **)&txtsize, 4096, sizeof(int));
...
//             // reads the file (up to MAX_BUF_SIZE)
//             fread((void *)txt, 1, size_in_bytes, fd); 
//             fclose(fd);
...
//             // creates the task
//             FTaskCL * task = new FTaskCL();
//             task->addIn(txt, size_in_bytes);
//             task->addOut(zip, size_in_bytes);
//             task->addOut(zipsize, sizeof(int));
//             task->addScalar(txtsize, sizeof(int));
//             task->setUserData((void *)filename);
...
//         char * zip = (char *)(task->out[0].ptr);
//         int * zipsize = (int *)(task->out[1].ptr);
//         char * filename = (char *)task->user_data;

//         // appends the suffix to the filename
//         char suffix[16] = "zip";
//         char _filename[256];
//         snprintf(_filename, 256, "zip/%s.%s", filename, suffix);

//         cout << "Drain got a result of " << *zipsize << " bytes " << endl; 
//         FILE * fd = fopen(_filename, "w");
//         if(fd == NULL) {
//             cout << "Error opening outfile" << endl;
//             exit(-1);
//         }
//         fwrite((void *)zip, 1, *zipsize, fd); 
//         fclose(fd);
