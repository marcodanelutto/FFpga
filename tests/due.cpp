...
int main(int argc, char * argv[]) {
  ...
  string kernelName1 = "krnl_vadd";
  string bitStream1  = "krnl_vadd.xclbin"; 
  string kernelName2 = "krnl_vsub";
  string bitStream2  = "krnl_vsub.xclbin"; 
  if(argc != 1) {
    kernelName1 = argv[1];
    bitStream1 = argv[2]; 
    kernelName2 = argv[3];
  ...
  cout << "Exec " << bitStream1 << " " << bitStream2 << endl; 
  ...
  ff_node_fpga s2(kernelName1, bitStream1, task_description);
  ff_node_fpga s3(kernelName2, bitStream2, task_description,false);
  ...
  p.add_stage(s3);	// add FPGA stage (FPGA not found -> this fails)
