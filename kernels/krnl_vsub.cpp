...
static void compute_sub(hls::stream<uint32_t>& in1_stream,
                        hls::stream<uint32_t>& in2_stream,
                        hls::stream<uint32_t>& out_stream,
                        int size) {
// The kernel is operating with vector of NUM_WORDS integers. The + operator performs
// an element-wise sub, resulting in NUM_WORDS parallel additions.
execute:
    for (int i = 0; i < size; i++) {
#pragma HLS LOOP_TRIPCOUNT min = c_size max = c_size
...
void krnl_vsub(uint32_t* in1, uint32_t* in2, uint32_t* out, int size) {
...
#pragma HLS dataflow
    ...
    compute_sub(in1_stream, in2_stream, out_stream, size);
